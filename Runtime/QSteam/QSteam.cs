...
#region 网络通信
		...
		{
			return SteamMatchmaking.GetNumLobbyMembers(lobby) > 1 || (SteamMatchmaking.GetNumLobbyMembers(lobby) == 1 && !SteamMatchmaking.GetLobbyMemberByIndex(lobby, 0).IsNull() && SteamMatchmaking.GetLobbyMemberByIndex(lobby, 0) != Id);
		}
		...
		{
			return SteamMatchmaking.GetLobbyOwner(steamID) == Id||( SteamMatchmaking.GetLobbyData(steamID, nameof(Application.productName)) == Application.productName &&
