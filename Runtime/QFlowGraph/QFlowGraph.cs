...
namespace QTool.FlowGraph
{
    ...
    public class QFlowGraph: QSerializeObject
	{
		...
		public IEnumerator RunIEnumerator(string startNode)
        {
			...
			if (curNode != null)
			{
				QDebug.Log("以[ " + curNode.ViewName + " ]为起点 运行流程图 [ " + Name + " ]");
				while (curNode != null)
				{
					...
					if (port != null)
					{
						...
						else
						{
							...
							{
								if (portObj.Node.State!= QNodeState.运行中)
								{
    ...
    public class QFlowPort : IKey<string>
    {
        ...
        public bool IsShowValue(int index=0)
        {
			if (FlowPort != null)
			{
				return IsList ? ValueType.GetElementType() != typeof(QFlow) : ValueType != typeof(QFlow);
			}
			else
			{
				return InputPort != null && !HasConnect(index)&& !InputPort.HasAutoValue;
			}
		}
		[QIgnore]
        ...
        public void ClearAllConnect(int index = -1)
        {
			if (index >= 0)
			{
    ...
    public class QFlowNode:IKey<string>
    {
        ...
        public QFlowPort AddPort(string key, Attribute portAttribute , string name="",Type type=null,QFlowPortAttribute FlowPort=null)
        {
			...
			port.ConnectType = port.FlowPort == null ?typeInfo.ElementType :QFlow.Type;
